...
var md5 = function (content) {
  return new Buffer(utility.md5(content)).slice(24);
};
...
exports.scan = function (view) {
  ...
  while ((block = reg.exec(view)) !== null) {
    for (var i = 0; i < block.length; i++) {
      ...
      var target = {};
      var arg = argReg.exec(find);
      if (arg !== null) {
        var assets = arg[0].replace(/\"/g, '').split(',');
        var assetExt = path.extname(assets[0]);
        target[assetExt] = assets[0];

        assets.shift();

        retVal.push({ target: target[assetExt], type: assetExt.replace('.', ''), assets: assets });
      }
    }
  }
  return retVal;
